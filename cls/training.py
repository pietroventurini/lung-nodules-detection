...
class ClassificationTrainingApp:
    def __init__(self, sys_argv=None):
        ...
        parser.add_argument('--dataset',
            help="Which dataset to feed to the model.",
            action='store',
            default='LunaDataset',
        )
        parser.add_argument('--model',
            help="Which model class name to use.",
            action='store',
            default='LunaModel',
        )
        ...
        parser.add_argument('--testing',
            help="Evaluate the specified model on the test set",
            default='',
        )
    ...
    def initModel(self):
        model_cls = getattr(cls.model, self.cli_args.model)
        model = model_cls()

        if self.cli_args.finetune and not self.cli_args.testing:
            d = torch.load(self.cli_args.finetune, map_location='cpu')
            # keep only layers with parameters (remove head_activation)
            model_blocks = [name for name, subm in model.named_children() if len(list(subm.parameters())) > 0]
            
            # take the last finetune_depth layers
            finetune_blocks = model_blocks[-self.cli_args.finetune_depth:]
            log.info(f"Finetuning from {self.cli_args.finetune}, blocks {' '.join(finetune_blocks)}")
            # Question: if finetune_depth == 2, should we load params for block4?? (model_blocks[-2])
            # if so we should write: if k.split('.')[0] not in finetune_blocks
            model.load_state_dict(
                {
                    k: v for k,v in d['model_state'].items() # model_state contains module.state_dict()
                    if k.split('.')[0] not in model_blocks[-1] # layers are named as layer_name.depth.weight
                },
                strict=False,
            )
            # set non-trainable weights for shallow layers
            for n, p in model.named_parameters(): 
                if n.split('.')[0] not in finetune_blocks:
                    p.requires_grad_(False)

        elif self.cli_args.testing:
            model_path = self.cli_args.testing
            d = torch.load(model_path, map_location='cpu') # try self.device instead of 'cpu'
    ...
    def initTrainDl(self):
        ...
        """
        ds_cls = getattr(cls.dsets, self.cli_args.dataset)

        train_ds = ds_cls(
            val_stride=10,
            isValSet_bool=False,
            ratio_int=1, # one-to-one balancing of training data
        )
    ...
    def initTensorboardWriters(self):
        if self.trn_writer is None:
            ...
            self.test_writer = SummaryWriter(log_dir=log_dir + '-test_cls-' + self.cli_args.comment)


    def main(self):
        log.info("Starting {}, {}".format(type(self).__name__, self.cli_args))
        if not self.cli_args.testing:
            ...
            for epoch_ndx in range(1, self.cli_args.epochs + 1):
                ...
                if epoch_ndx == 1 or epoch_ndx % validation_cadence == 0:
                    ...
                    self.saveModel(
                        'mal' if self.cli_args.malignant else 'cls', # FIXME: before it was just 'cls'
                        epoch_ndx, 
                        score == best_score
                    )
        else: # in case of testing
            test_dl = self.initTestDl()
            testMetrics_t = self.doValidation(1, test_dl)
            score = self.logMetrics(1, 'test', testMetrics_t)
    ...
    def doValidation(self, epoch_ndx, val_dl):
        with torch.no_grad():
            ...
            batch_iter = enumerateWithEstimate(
                val_dl,
                "E{} Validation ".format(epoch_ndx) if not self.cli_args.testing else "Evaluate performances on test set",
    ...
    def computeBatchLoss(self, batch_ndx, batch_tup, batch_size, metrics_g, augment=True):
        ...
        if augment:
            input_g = cls.model.augment3d(input_g)

        # logits = output of last linear layer
        # probability = output of softmax function
