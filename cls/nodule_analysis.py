...
from seg.model import UNetWrapper
...
logging.getLogger("seg.dsets").setLevel(logging.WARNING)
logging.getLogger("cls.dsets").setLevel(logging.WARNING)
...
class NoduleAnalysisApp:
    def __init__(self, sys_argv=None):
        ...
        parser.add_argument('--cls-model',
            help="Which model class name to use for the classifier.",
            action='store',
            default='LunaModel',
        )
    ...
    def initModels(self):
        ...
        model_cls = getattr(cls.model, self.cli_args.cls_model)
        cls_model = model_cls()
        ...
        if self.cli_args.malignancy_path:
            model_cls = getattr(cls.model, self.cli_args.malignancy_model)
    ...
    def main(self):
        ...
        for _, series_uid in series_iter:
            ct = getCt(series_uid)
            mask_a = self.segmentCt(ct, series_uid) # run segmentation model on ct

            candidateInfo_list = self.groupSegmentationOutput(series_uid, ct, mask_a) # group flagged voxels
            classifications_list = self.classifyCandidates(ct, candidateInfo_list) # run nodule classifier on candidates

            if not self.cli_args.run_validation: # print individual information
                print(f"found nodule candidates in {series_uid}:")
                for prob, prob_mal, center_xyz, center_irc in classifications_list:
                    if prob > 0.5: # candidate with probability of being a nodule > 50% (lower it to reduce risk of false negatives)
            ...
            if series_uid in candidateInfo_dict: # if we have ground thruth data, print confusion matrix
    ...
    def classifyCandidates(self, ct, candidateInfo_list):
        """ Classify candidates into nodule vs. non-nodule
        """
        # make a data loader from the list of candidates
        cls_dl = self.initClassificationDl(candidateInfo_list)
        classifications_list = []
        for batch_ndx, batch_tup in enumerate(cls_dl):
            input_t, _, _, series_list, center_list = batch_tup

            input_g = input_t.to(self.device) # move to GPU
            with torch.no_grad():
                _, probability_nodule_g = self.cls_model(input_g) # run input through the nodule vs. non-nodule network
                if self.malignancy_model is not None:
                    _, probability_mal_g = self.malignancy_model(input_g) # run through malignancy model
                else:
    ...
    def segmentCt(self, ct, series_uid):
        """ Feed each slice of `ct` into the segmentation model.
        """
        with torch.no_grad(): # no need to build the graph
            output_a = np.zeros_like(ct.hu_a, dtype=np.float32)
            seg_dl = self.initSegmentationDl(series_uid) # get data loader to loop over our CT in batches
            for input_t, _, _, slice_ndx_list in seg_dl:

                input_g = input_t.to(self.device) # move input to GPU
                prediction_g = self.seg_model(input_g)

                for i, slice_ndx in enumerate(slice_ndx_list):
                    output_a[slice_ndx] = prediction_g[i].cpu().numpy() # copy each element to output array
    ...
    def groupSegmentationOutput(self, series_uid,  ct, clean_a):
        """ Convert the output of the segmentation model into
        a list of candidates, by grouping together adjacent voxels sharing 
        the same segmented area.
        """
        # assigns each voxel the label of the group it belongs to
        candidateLabel_a, candidate_count = measurements.label(clean_a)
        # gets center of mass for each group as idx,row,col coordinates
        ...
        for i, center_irc in enumerate(centerIrc_list):
            ...
            # use placeholders for isNodule_bool, hasAnnotation_bool, isMal_bool, diameter_mm
