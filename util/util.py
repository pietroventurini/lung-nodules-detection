...
def irc2xyz(coord_irc, origin_xyz, vxSize_xyz, direction_a):
    """ Transforms a point expressed in (index,row,column) coordinates
    into (x,y,z) coordinates.
    """
    cri_a = np.array(coord_irc)[::-1] # swap the order (irc --> cri)
    origin_a = np.array(origin_xyz)
    vxSize_a = np.array(vxSize_xyz)
    # 1. scale the indices with the voxel sizes
    # 2. matrix multiply with the directions
    # 3. add the offset for the origin
    coords_xyz = (direction_a @ (cri_a * vxSize_a)) + origin_a
    return XyzTuple(*coords_xyz)

def xyz2irc(coord_xyz, origin_xyz, vxSize_xyz, direction_a):
    """ Transforms a point expressed in (x,y,z) coordinates
    into (index,row,column) coordinates.    
    """
    origin_a = np.array(origin_xyz)
    vxSize_a = np.array(vxSize_xyz)
    coord_a = np.array(coord_xyz)
    # 1. subtract the offset for the origin
    # 2. inverse matrix multiplication with the directions
    # 3. scale the indices with the voxel sizes
    cri_a = ((coord_a - origin_a) @ np.linalg.inv(direction_a)) / vxSize_a
    cri_a = np.round(cri_a) # discretize
...
def enumerateWithEstimate(
    ...
    :param backoff: This tells how many iterations to skip before
